...
| 2026-08-03 | Added plugins/device/jackal/ — Clearpath Jackal (ROS2) device plugin via rosbridge/henosync_sdk.rosbridge.ensure_reactor. VICON (default) or GPS positioning (fixed gps+battery+imu capabilities; optional camera+lidar, topics overridable in config since Jackal is a modular platform).                                                                                                                                                                                                                                                                                                                                             |
...
| 2026-08-18 | auto-navigate: implemented move-to-marker — single robot, haversine arrival detection (2m default), configurable timeout; REQUIRED_CAPABILITIES reduced to MOVE_2D only (GPS not required); fully abstracted via DeviceProxy |
...
| 2026-08-18 | marker_manager: added get_marker(id) method |
| 2026-08-18 | frontend: added marker_select field type; MarkerSelectField component in MissionPage renders live dropdown from markerStore |
| 2026-08-31 | turtlebot3: fixed cmd_move_to — replaced stale self.\_local_to_gps() call (removed in SDK refactor) with inline equirectangular conversion |
| 2026-08-31 | operation_manager.start_operation: auto-clean completed (done) tasks when plugin tries to restart — prevents "already running" 400 after a timed-out or failed operation |
| 2026-08-31 | turtlebot3: added detailed INFO logging for navigation diagnostics (per-tick position/bearing/heading/distance/error) |
| 2026-08-31 | Wired arrival_radius_m end-to-end: SDK interfaces.py dispatcher forwards it; DeviceProxy.move_to() accepts and passes it in params; turtlebot3 cmd_move_to accepts it and uses it instead of hardcoded ARRIVAL_THRESHOLD_M |
| 2026-08-31 | turtlebot3: cmd_move_to — added try/finally to guarantee zero-velocity publish on CancelledError; uses node.position.heading (VICON world-frame yaw) instead of odom heading in local-frame mode so bearing and heading are in the same reference frame |
| 2026-08-31 | device_proxy: added local_origin property so control plugins can read a device's home origin for coordinate conversion |
| 2026-08-31 | VICONMap: added scroll/pinch zoom — zoom state + zoomRef (avoids stale closure in rapid wheel events); viewBox size = VB/zoom; getSvgPerPx and eventToVicon account for zoom; handleWheel zooms toward cursor by adjusting panOffset; range 0.5–20×; applies on all pages |
| 2026-08-31 | VICONMap: zoom-invariant element sizes — all fixed-screen-size SVG values (dot radii, stroke widths, font sizes, label offsets, dash arrays) multiplied by s=1/zoom so markers, robot dots, zone borders, and labels stay the same visual size regardless of zoom level |
| 2026-08-31 | vicon_manager: added vicon_heading (yaw radians) to TelemetryFrame.custom alongside vicon_x/vicon_y; VICONMap robot dots now show a heading arrow (white line + arrowhead) pointing in the robot's forward direction; arrow hidden when no heading available |
| 2026-09-02 | Added plugins/device/crazyflie/ — Crazyflie 2.x plugin (Milestone 1: telemetry only); cflib direct via Crazyradio PA USB dongle; VICON positioning via vicon_manager; battery via LogConfig pm.vbat (4.2V full / 3.0V empty); asyncio.Event bridge for cflib callbacks; \_ensure_drivers() module-level singleton with threading.Lock; no movement (get_safe_state no-op); added cflib to setup.bat and setup.sh |
| 2026-09-07 | teleop: added max_speed config option (number, 0.01–1.0, optional) — scales linear and angular velocity; blank/omitted defaults to 1.0 (full speed) |
| 2026-09-07 | jackal: replaced Nav2 goal_pose cmd_move_to with proportional velocity controller (same algorithm as turtlebot3); removed goal_pose_pub from \_NodeState and connect(); removed goal_pose_topic manifest field; added MAX_LINEAR_VEL/ANGULAR_VEL/LINEAR_GAIN/ANGULAR_GAIN/ARRIVAL_THRESHOLD_M/ARRIVAL_POLL_S class constants |
| 2026-09-07 | jackal manifest: removed home_lat, home_lon, max_linear_speed, max_angular_speed config fields; added imu_topic override field (default sensors/imu_0/data); jackal plugin.py: removed VICON-mode home_lat validation; fixed IMU topic to use configurable imu_topic |
| 2026-09-07 | DevicePanel and DeviceDetailPanel: errored/offline devices now show category and capabilities from plugin manifest (useDevicePlugins fallback) instead of "Unknown"; battery bar hidden when status=error |
| 2026-09-07 | Added trailStore (sliding window 500pts, GPS [lon,lat] + VICON [x_m,y_m]); useTrailRecorder hook in App.tsx records positions while any operation is running, clears on new operation start; NodeTrails MapLibre component renders GPS trails on HomePage and MissionPage; VICONMap renders VICON trails as zoom-invariant SVG polylines |
| 2026-08-31 | Added MarkerManager.get_marker(id) — was missing, caused auto-navigate to crash immediately on start |
| 2026-08-31 | turtlebot3: fixed cmd_move_to — replaced stale self.\_local_to_gps() call (removed in SDK refactor) with inline equirectangular conversion |
| 2026-08-31 | operation_manager.start_operation: auto-clean completed (done) tasks when plugin tries to restart — prevents "already running" 400 after a timed-out or failed operation |
| 2026-08-31 | auto-navigate, turtlebot3: added detailed INFO logging for navigation diagnostics (marker raw coords, GPS conversion, per-tick position/bearing/heading/error) |
| 2026-08-31 | auto-navigate: fixed VICON navigation — VICON markers store lon=x_m/lat=y_m (raw metres); plugin now converts to real GPS via device.local_origin before calling move_to so haversine distance is in actual metres; timeout now enforced via asyncio.wait_for wrapping device.move_to; stop() calls device.stop() immediately |
| 2026-08-31 | Wired arrival_radius_m end-to-end: SDK interfaces.py dispatcher forwards it; DeviceProxy.move_to() accepts and passes it in params; turtlebot3 cmd_move_to accepts it and uses it instead of hardcoded ARRIVAL_THRESHOLD_M; auto-navigate reads it from config and passes to device.move_to(); config_schema min lowered to 0.05m, default to 0.3m |
| 2026-08-31 | turtlebot3: cmd_move_to — added try/finally to guarantee zero-velocity publish on CancelledError; uses node.position.heading (VICON world-frame yaw) instead of odom heading in local-frame mode so bearing and heading are in the same reference frame |
| 2026-08-31 | device_proxy: added local_origin property so control plugins can read a device's home origin for coordinate conversion |
| 2026-08-24 | VICON map mode: GPS/VICON toggle pill on HomePage; VICONSetupModal (shape + dimensions, localStorage); VICONMap SVG component (boundary rect/circle, robot dots by raw x/y); vicon_manager emits vicon_x/vicon_y in TelemetryFrame.custom for frontend positioning |
| 2026-08-24 | Fixed GPS→VICON blue-screen crash: GPS MissionMap now always mounted in both HomePage and ZonesPage; VICON overlay at zIndex:2 with solid #0D0D0D background covers it — avoids maplibre WebGL lifecycle teardown that caused the crash; ZonesPage updated to same always-mounted pattern |
