...
- `_resolve_target()` resolves the target via `context.marker_manager.get_marker()` / `context.zone_manager.get_zone()` (zone centroid = circle center, or average of polygon points) — **once per step, not once per device** (see multi-device below). Returns `(kind, a, b)`: `kind=="gps"` means `(a, b)` is a real `(lat, lon)` (GPS-mode marker/zone), used as-is. `kind=="local"` means `(a, b)` is `(x_m, y_m)` in a VICON arena's own metres (`map_mode=="vicon"` — see GPS vs VICON targets below) — every device converts it independently at dispatch time, since there's no shared real-world coordinate for a VICON arena.
...
**GPS vs VICON targets (2026-08-31):** a marker/zone's `map_mode` decides what its `lat`/`lon` fields mean — real WGS84 for `"gps"`, raw VICON arena metres for `"vicon"` (`VICONMap.tsx`'s convention: `lon=x_m, lat=y_m`). There is deliberately no arena-wide real-world anchor anywhere in the system (an earlier version added one — `vicon_manager.origin`/`set_origin()`, converting VICON zones/markers to real WGS84 at creation time — reverted; VICON stays local-only, matching how `VICONMap.tsx` itself works). Instead:
- `self._current_target: Optional[tuple[float, float]]` (GPS) and `self._current_local_target: Optional[tuple[float, float]]` (VICON local) — exactly one is set at a time, by `_execute_step()` based on `_resolve_target()`'s `kind`.
- `_run_on_devices_local(devices, local_target, context)` mirrors `_run_on_devices()` for a `"local"` target — dispatches every device concurrently via `_go_to_local_waypoint_tracked()`.
- `_go_to_local_waypoint_tracked(device, x_m, y_m, context)` converts `(x_m, y_m)` to that device's own `(lat, lon)` via `device.local_to_gps()` (`DeviceProxy`'s existing local-frame conversion, using the device's own `local_origin` — the same mechanism `vicon_manager` already uses to give a VICON-tracked device its own synthetic real position), then delegates to `_go_to_waypoint_tracked()`. Two devices with different `local_origin`s convert the *same* VICON target to *different* real coordinates — verified in `test_area_coverage_exec.py`.
...
- `AutoNavigatePlugin.on_device_joined(device)` — if a `MOVE_TO_MARKER`/`MOVE_TO_ZONE` target is currently active (`self._current_target` for GPS or `self._current_local_target` for VICON, set by `_execute_step()`), fires the new device toward it as a background task (`asyncio.create_task`, via `_go_to_waypoint_tracked()` or `_go_to_local_waypoint_tracked()` as appropriate), concurrently with whatever's already in flight. Needs `self._context` — stashed in `start()`, since the `ControlPlugin` base class declares `self._context` but `operation_manager` never actually populates it, and `on_device_joined()`'s signature (part of the `ControlPlugin` ABC, shared by all control plugins) doesn't receive `context` as a parameter.
...
- `_generate_coverage_paths(zone, num_robots, spacing_m, angle_deg)` (module-level, pure geometry) — returns `(kind, paths)`. Branches on `zone.map_mode`: GPS zones go through `_zone_polygon_and_origin()` (lat/lon → local-XY relative to the zone's own centroid; circle zones approximated as a `CIRCLE_APPROXIMATION_SIDES`-gon) and the result is converted back to lat/lon at the end (`kind="gps"`); VICON zones go through `_vicon_zone_polygon()` instead — `zone.points`/`center` ARE ALREADY local-XY (arena metres, `lon=x_m, lat=y_m`), so no origin/projection step exists at all, and the output stays in that same local frame (`kind="local"`) for `_execute_area_coverage()` to convert per device. Both paths share `_sweep_bands(polygon, num_robots, spacing_m, angle_deg)` for the actual geometry: rotates so `angle_deg` (0=east, 90=north, matching the field's existing doc) aligns with +x (`_rotate`/`_unrotate`), slices the rotated bounding box into `num_robots` bands of **equal area** via `_equal_area_band_bounds()` (not equal height — see below), and generates a boustrophedon (alternating left-right/right-left) sweep within each band at `spacing_m` line spacing. Per-line extent is found by **sampling** (`_inside_extent` + `_point_in_polygon_xy`, ray-casting) rather than exact polygon-edge intersection — a scan line's covered extent is the outer span of inside samples, not a per-gap breakdown, so a sharply concave zone can get a slightly optimistic or stair-stepped sweep near inward corners. Sample count is capped (`MAX_COVERAGE_SAMPLES=20000`, coarsened rather than failed if exceeded — same spirit as `mission_engine`'s `MAX_ITERATIONS`). Raises `ValueError` (caught by `_execute_area_coverage`, surfaced via `context.send_alert`) for zero robots, non-positive spacing, a zone too small to have any extent, or unusable zone geometry.
...
- `_run_coverage_path(device, waypoints, kind, start_position, context)` — registers the device in `_current_devices` once for the whole path (not per-waypoint, so `stop()`/the collision guard treat a mid-sweep device the same as one mid-single-waypoint nav), walks `waypoints` in order via `_navigate_one_waypoint()`, converting each one through `device.local_to_gps()` first when `kind=="local"` (VICON — one waypoint at a time, since there's no shared coordinate to convert the whole path through at once), then navigates to `start_position` if one was captured (always real lat/lon, captured via `device.get_gps_data()` — unaffected by `kind`, since a VICON-tracked device already reports its own synthetic real position). `_device_status` reports sweep progress (`"Sweeping — waypoint 3/8"`) and the final outcome.
...
Verified via simulation (`test_area_coverage.py`, `test_area_coverage_exec.py` — not committed, scratch scripts): geometry — rotate/unrotate round-trips, point-in-polygon edge cases, a 3-way split of a square zone stays within its bounding box, circle-zone approximation, `num_robots=1` degenerate case, `ValueError` on zero robots/non-positive spacing, a 4-way split of an L-shaped (concave) zone where each band's actual area is independently re-measured on a 400×400 grid and confirmed within 0.3% of an equal share (vs. 29.4% for a naive equal-height split), and — added with the GPS/VICON split — a VICON-mode zone confirmed to return `kind="local"` with every waypoint staying at raw arena-metres scale (not accidentally WGS84-projected). Execution — two fake devices given genuinely different sweep paths (not one shared target), each returns to its own captured start position, per-device status reflects sweep progress and completion, `_current_devices`/`_device_target` are cleared after completion, a device with `_stop_requested` already set issues zero `move_to()` calls and reports "Stopped", the collision guard still detects and pauses the farther-from-its-own-target device when two devices' fake positions are placed within `MIN_SEPARATION_M` during a coverage run, and — for VICON targets — two fake devices with different `local_origin`s convert the identical MOVE_TO_ZONE/MOVE_TO_MARKER/AREA_COVERAGE target to genuinely different real coordinates (proving no shared arena origin leaked back in), each landing near its OWN home and returning to its OWN captured start. Not yet tested against real devices, `turtlebot3`, or `ue-sim` live.
...
- Positions are WGS84 GPS (lat/lon/alt) for **GPS-mode** zones/markers/devices — DeviceProxy handles conversion for local-frame (odometry) devices. **VICON-mode** zones/markers (`map_mode == "vicon"`) are the deliberate exception: `zones.py`/`markers.py` store whatever `VICONMap.tsx` sends verbatim — raw VICON arena metres, not real coordinates (`lon=x_m, lat=y_m` convention) — there is no arena-wide real-world anchor anywhere in the system. `auto-navigate` is the one consumer that needs to act on these; it keeps a VICON-mode target/zone in that local frame throughout its own geometry and only converts to lat/lon per DEVICE, via that device's own `local_origin` (`DeviceProxy.local_to_gps()`), right before dispatch — see the Auto Navigate section. (2026-08-31: an earlier version of this added a shared "arena origin" concept to convert VICON coordinates to real WGS84 at creation time — reverted; VICON deliberately stays local-only.)
...
| 2026-08-31 | Reverted the arena-origin approach from the previous entry — user feedback: VICON should stay local-only, matching VICONMap.tsx's own design (no lat/lon anywhere in it) and not introduce a new arena-wide coordinate concept. Removed: `vicon_origin` table, `vicon_manager.origin`/`set_origin()`/`_load_origin()` (`local_to_gps` reverted to private `_local_to_gps`, its only remaining caller), `POST /api/vicon/origin` + `home_lat`/`home_lon` from `GET /api/vicon/connection`, the conversion-on-write blocks in `zones.py`/`markers.py`, the "Arena Origin" UI in `App.tsx`'s VICON panel, `VICONMap.tsx`'s `gpsToLocal()`/`homeLat`/`homeLon` (reverted to direct raw-metres passthrough — the teammate's pan feature added in the same file was unaffected, since it works purely on SVG viewBox translation), and the `homeLat`/`homeLon` wiring in `ZonesPage.tsx`/`HomePage.tsx`/`MissionPage.tsx`. Replaced with local-only handling in `auto-navigate` itself (the one consumer that actually needs real coordinates): `_generate_coverage_paths()`/`_resolve_target()` now return a `(kind, ...)` tag — `"gps"` (real lat/lon, used as-is) or `"local"` (VICON arena metres, kept in that frame through all of `auto-navigate`'s own geometry — new `_vicon_zone_polygon()` + `_sweep_bands()`, the latter extracted from the old `_generate_coverage_paths()` body so GPS and VICON zones share the same rotate/equal-area/boustrophedon core). A `"local"` target is converted to real lat/lon per DEVICE, at the point of dispatch, via the device's own `local_origin` (`DeviceProxy.local_to_gps()` — the exact mechanism `vicon_manager` already uses to give a VICON-tracked device its own synthetic position) — new `self._current_local_target`, `_run_on_devices_local()`, `_go_to_local_waypoint_tracked()`; `_run_coverage_path()` gained a `kind` param converting one waypoint at a time. `on_device_joined()` now checks both `_current_target` and `_current_local_target`. Verified: two fake devices with different `local_origin`s convert the identical VICON target to different real coordinates for MOVE_TO_ZONE, MOVE_TO_MARKER, and AREA_COVERAGE alike — see `test_area_coverage_exec.py` |
| 2026-09-06 | auto-navigate config form: replaced the single free-text `target_id` field ("Marker or Zone ID", placeholder "From the map panel") with proper `marker_id`/`zone_id` dropdowns — there was no way to actually find a zone/marker's ID anywhere in the UI (`ZoneListPanel`/`ZoneDetailPanel` never rendered `zone.id`), so the field was unusable as shipped. Added `zone_select`/`marker_select` to `PluginConfigField.type` and two new components in `ConfigField.tsx` (`ZoneSelectField`/`MarkerSelectField`, mirroring the existing `DeviceSelectField` pattern) reading live `useZoneStore`/`useMarkerStore` — both stores stay populated regardless of which page is visible, since `ZonesPage` (which calls `useZones()`/`useMarkers()`) is always mounted, just hidden via `display:none`, same as every other page. Dropdown options show `"{name} ({map_mode})"` so GPS vs VICON zones/markers are distinguishable. `_parse_steps()` now reads `marker_id`/`zone_id` directly instead of one generic `target_id` |
| 2026-09-07 | jackal: fixed `cmd_move_to()` returning success the instant the Nav2 `goal_pose` was *published*, not when the robot actually arrived — fatal for auto-navigate's AREA_COVERAGE, which treats a successful `move_to()` as "arrived, send the next waypoint": it would fire off the entire multi-waypoint sweep almost instantly, each goal superseding the last before the robot had gone anywhere. `cmd_move_to()` now blocks: after publishing the goal it polls `node.position` (kept live by `vicon_manager`) against the target, converting back to local metres via the same equirectangular formula `cmd_return_home()` already used, until within `GOAL_ARRIVAL_THRESHOLD_M=0.5m`, `GOAL_TIMEOUT_S=120s` elapses, or stop is requested — same shape as `turtlebot3.cmd_move_to()`'s own blocking loop, except Nav2 does the actual driving here, not this method. Added `_NodeState.stop_requested`, set by `cmd_stop()`/`get_safe_state()` (mirrors turtlebot3) so an in-flight wait can be interrupted — still does NOT cancel the underlying Nav2 goal itself (documented as a pre-existing, unchanged KNOWN LIMITATION). GPS-frame rejection (`x`/`y` both `None`) is unchanged, including the "not implemented" substring in its message that auto-navigate's fallback-detection relies on. Verified via simulation: arrival, non-instant return, timeout, and stop-interruption |
